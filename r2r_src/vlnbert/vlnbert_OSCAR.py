...
class BertImgModel(BertPreTrainedModel):
    ...
    def forward(self, mode, input_ids, token_type_ids=None, attention_mask=None,
        ...
        if mode == 'visual':
            ...
            encoder_outputs, layer_head_attn = self.encoder(mode, concat_embedding_output,
        ...
        return outputs, layer_head_attn
...
class VLNBert(BertPreTrainedModel):
    ...
    def forward(self, mode, input_ids, token_type_ids=None, attention_mask=None,
        ...
        outputs, layer_head_attn = self.bert(mode, input_ids, position_ids=position_ids, token_type_ids=token_type_ids,
        ...
        if mode == 'language':
            ...
            head_prob = nn.Softmax(dim=-1)(layer_head_attn)
            obj_token_attn = nn.Softmax(dim=-1)(outputs[4])[:,:,1:,1:-self.config.directions]
            obj_token_attn = obj_token_attn.mean(dim=1)

            language_attentions = outputs[2][:, :, 1:-self.config.directions]
            visual_attentions = outputs[2][:, :, -self.config.directions:]

            lang_head_prob = torch.mean(head_prob[:, :, :, 1:-self.config.directions], dim=-1)
            visul_head_prob = torch.mean(head_prob[:, :, :, -self.config.directions:], dim=-1)
            ...
            return state_proj, visual_attention_scores, lang_head_prob, visul_head_prob, obj_token_attn